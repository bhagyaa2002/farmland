...
const AdminFertilizerCard = ({ id, data }) => {
  ...
  const { deleteCropMarket, user, getCropMarket, deleteFertilizerMarket } =
    useUserAuth();
  ...
  return (
    <>
      <div className="admincardmainfertilizer">
        ...
        <div className="editFertilizerquantity">
          <div className="admincardheaddingfer1">
            <h1>{data.quantity}</h1>
          </div>
          <div className={data.NoOfItemsAvailable>10?"admincardheaddingfer3":"admincardheaddingfer2"}>
          <h1>{data.NoOfItemsAvailable?data.NoOfItemsAvailable+" Qty Left":null}</h1>
          </div>
        </div>
        ...
        <div className="adminpricefer">
          ...
          <h3 style={{ color: "red", marginLeft: "4px" }}>
            {Math.round((offer / mrp) * 100 - 100).toFixed(2)}%
          </h3>
        </div>
